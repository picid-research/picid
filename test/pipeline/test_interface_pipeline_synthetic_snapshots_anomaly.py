"""Pipeline snapshot tests for anomaly detection using synthetic data only.

Compares current run to reference metrics. Uses anomaly_detection.npz fixture.
...
# Anomaly detection uses Isolation Forest only (configs/model_configs/anomaly_detection/)
EXPERIMENTS = [
...
def test_pipeline_snapshot_matches_reference(task_type, exp_base, model):
    ...
    results = interface.train(run_name='test',
                              model=None,
